const lcl = require('cli-color'),
    ...
    getDownloadURL = require('./github'),
    fetchDownload = require('./download');
...
async function downloadCore() {
    ...
    if (process.env.UPDATE == 'false' && version.version != '0.0.0') {
        console.log(lcl.yellow("[YTDLP Download - Warn]"), "Auto-update disabled, skipping update check... (This isn't recomended as TikTok often changes their website and API so old versions of YTDLP will break)");
        return {
            success: true
        }
    }
    ...
    if (githubVersion.success) {
        ...
        console.log(lcl.red("[Github API - Error]"), "Failed to get latest version from github");
        return {
            success: false
        }
    }
