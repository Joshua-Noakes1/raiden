...
async function fetchDownload(URL, platform) {
    try {
        ...
        // write to file
        await writeFileSync(path.join(__dirname, '../', '../', 'ytdlp', 'bin', `ytdlp-exec${platform}`), YTDLPExec);

        // make file executable for user 
        if (process.platform == 'darwin' || process.platform == 'linux') await exec(`chmod +x ${path.join(__dirname, '../', '../', 'ytdlp', 'bin', `ytdlp-exec${platform}`)}`);
