...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            var tikTokVideos = await getTikTok(url);
            if (!tikTokVideos.success) {
            ...
            // throw new Error("Block all;");

            for (var tikTokVideo of tikTokVideos.video) {
                // build video object
                var videoObject = {
                    "account": {
                        "name": `${tikTokVideo.creator}`,
                        "username": `${tikTokVideo.uploader}`,
                        "id": `${tikTokVideo.uploader_id}`,
                        "uuid": `${tikTokVideo.uploader_url.toString().split("/@")[1]}`,
                        "url": `${tikTokVideo.uploader_url}`,
                    },
                    "video": {
                        "id": `${tikTokVideo.id}`,
                        "description": `${tikTokVideo.title.toString().replace(/\s+$/, '')}`,
                        "url": `${tikTokVideo.uploader_url}/video/${tikTokVideo.id}`,
                        "uploadTime": await getTime(Math.floor(tikTokVideo.timestamp * 1000)),
                        "stats": {
                            "views": `${tikTokVideo.view_count.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`,
                            "likes": `${tikTokVideo.like_count.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`,
                            "comments": `${tikTokVideo.comment_count.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`,
                            "reposts": `${tikTokVideo.repost_count.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`,
                        },
                        "media": {
                            "watermark": {
                                "url": "",
                    ...
                    "audio": {
                        "track": `${tikTokVideo.track || "Unknown"}`,
                        "artist": `${tikTokVideo.artist || "Unknown"}`,
                        "album": `${tikTokVideo.album || "Unknown"}`,
                    }
                }
                ...
                var watermarkVideo = await findVideo("watermark", tikTokVideo.formats);
                ...
                var cleanVideo = await findVideo("clean", tikTokVideo.formats);
                ...
                var dynamicThumbnail = await findThumbnail(tikTokVideo.thumbnails, "dynamic");
                ...
                var staticThumbnail = await findThumbnail(tikTokVideo.thumbnails, "static");
            ...
            // account embed - No text used
            if (tikTokVideos.video.length > 1) {
                try {
                    // download account avatar
                    console.log(lcl.blue("[Account - Info]"), "Downloading account avatar...");
                    var accountThumbnail = await downloadMedia(tikTokVideos.user.avatar, "png");
                    var attachemtnts = [];
                    if (accountThumbnail.success) {
                        attachemtnts.push({
                            "name": `${accountThumbnail.uuid}.png`,
                            "attachment": accountThumbnail.filePath
                        });
                    }
                    console.log(lcl.green("[Account - Success]"), "Downloaded account avatar.");

                    console.log(lcl.blue("[Video - Info]"), "Building account embed...");
                    var accountEmbed = new EmbedBuilder()
                        .setTitle(`@${tikTokVideos.user.username} (${tikTokVideos.user.name})`)
                        .setURL(`https://www.tiktok.com/@${tikTokVideos.user.url}`)
                        .setColor(colorHex())
                        .setFooter({
                            text: `@${tikTokVideos.user.username}`
                        });
                    console.log(lcl.green("[Video - Success]"), "Built account embed.");
                    ...
                    console.log(err);
                    console.log(lcl.red("[Video - Error]"), "Something went wrong while sending account embed.");
                }
            }
