async function findVideo(type, formats) {
    ...
    for (const videos in formats) {
        if (type === "watermark") {
            ...
            if (formats[videos].format_note.toString().toLowerCase().includes("direct") && formats[videos].source_preference >= -1 && !videoFormat.clean.success && !formats[videos].format_note.toString().toLowerCase().includes("(api)")) {
