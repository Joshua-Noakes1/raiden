...
async function downloadMedia(url, ext, headers) {
    try {
        ...
        var mediaPath = path.join(__dirname, "download", `${mediaUUID}`);
        await writeFileSync(`${mediaPath}.${ext}`, mediaBuffer);

        var fileSize = await statSync(`${mediaPath}.${ext}`).size;

        console.log(lcl.green("[Media - Success]"), "Media downloaded successfully.");
        return {
            success: true,
            mediaPath,
            mediaUUID,
