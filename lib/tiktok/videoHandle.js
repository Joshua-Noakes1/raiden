const lcl = require('cli-color'),
    ...
    videoYTDLP = require('../ytdlp/exec'),
    extractVideoData = require('./videoLib/extractVideoData'),
    downloadMedia = require('./downloadMedia'),
...
async function videoHandle(interaction, url) {
    ...
    await asyncForEach(videoDataResult.videos, async (video, index, array) => {
        ...
        if (videoData.videos.watermarked.watermarkedSkipped) {
            attachments.push({
        ...
        try {
            ...
            if (checkedAttachments.attachmentsTooBig) await interaction.followUp({
            ...
            console.log(error);
            await interaction.followUp({
                content: `**Only sending meta data, discord is having an issue uploading the videos**${array.length > 1 ? ` (Video ${index + 1} of ${array.length})` : ''}`,
                ephemeral: true
            });
            await interaction.followUp({
                embeds: [videoEmbed]
